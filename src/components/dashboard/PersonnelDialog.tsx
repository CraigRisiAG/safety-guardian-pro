...
function PersonnelRow({ person, buildings, allPersonnel, isEditing, onEdit, onSave, onCancel, onDelete, getParticipationColor }: PersonnelRowProps) {
  ...
  const staffCodeError = isEditing ? validateStaffCode(editData.staffCode) : null;

  const editDialog = (
    <Dialog open={isEditing} onOpenChange={(o) => { if (!o) onCancel(); }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  disabled={!editData.buildingAccess[0]}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select floor" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none">No primary floor</SelectItem>
                    {allFloors
                      .filter(f => f.buildingId === editData.buildingAccess[0])
                      .map((floor) => (
                        <SelectItem key={floor.id} value={floor.id}>
                          {floor.name}
                        </SelectItem>
